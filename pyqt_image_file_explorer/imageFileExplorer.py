...
class RemoveCellPractice(QWidget):
    def __init__(self):
        super().__init__()
        self.__initUi()
    ...
    def __initUi(self):
        ...
        countBtn = QPushButton('갯수 보기')
        countBtn.clicked.connect(self.__tableWidget.count)

        lay = QHBoxLayout()
        lay.addWidget(addBtn)
        lay.addWidget(delBtn)
        lay.addWidget(countBtn)
        lay.setContentsMargins(0, 0, 0, 0)
    ...
    def __add(self):
        dirname = QFileDialog.getExistingDirectory(self, '폴더 열기', '')
        if dirname:
...
if __name__ == "__main__":
    ...
    removeCellPractice = RemoveCellPractice()
    removeCellPractice.show()
    app.exec_()